 ...
 */
@Component("im4JavaThumbGenerator")
public class Im4JavaThumbGenerator extends AbstractThumbGenerator {
    @Override
    protected String generate(String imageFullName, ThumbGeneratorMode mode, int width, int height) {
        try {
            ...
            imOperation.addImage(imageFullName);
...
//            double scaling = (double) oWidth / (double) oHeight;
            ...
            if(mode == ThumbGeneratorMode.ScaleByWidthAndHeight) {
                ...
                imOperation.sample(width, height);
                imOperation.gravity("center");
                imOperation.extent(width, height);
            }else if(mode == ThumbGeneratorMode.ScaleByWidth) {
                imOperation.sample(width, null);
            }else if(mode == ThumbGeneratorMode.ScaleByHeight) {
                imOperation.sample(height, null);
            }

            imOperation.addImage(thumbFullName);
